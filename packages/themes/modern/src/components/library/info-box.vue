<template>
  <div class="bg-white rounded-lg shadow-lg p-6 mb-6 border border-gray-100 flex items-center gap-6 hover:shadow-lg hover:shadow-blue-500/5 transition-all duration-300 group cursor-pointer active:scale-95">
    <div :class="[getModernColor(), 'w-14 h-14 flex shrink-0 items-center justify-center rounded-lg transition-transform group-hover:rotate-6 shadow-lg shadow-current/10']">
    ...
    <div class="flex-1 min-w-0">
        <h5 class="text-xs font-black text-gray-900 uppercase tracking-widest leading-tight">
            <a v-if="link !== ''" :href="link" class="hover:text-blue-600 transition-colors block truncate" v-html="content"></a>
            <span v-else v-html="content" class="block truncate"></span>
        </h5>
